...
Desktop signing and publication
-------------------------------

The release workflow publishes the Universal macOS disk image and Windows Qt 6
installer directly to the GitHub release after both builds and test suites pass.
It always publishes SHA-256 checksum files.

For signed Windows installers, configure these GitHub Actions secrets:

- `WINDOWS_CERTIFICATE_BASE64`: base64-encoded PFX certificate
- `WINDOWS_CERTIFICATE_PASSWORD`: PFX password

For a Developer ID-signed and notarized macOS disk image, configure:

- `APPLE_CERTIFICATE_BASE64`: base64-encoded PKCS#12 certificate
...
When Apple credentials are absent, local and CI builds retain the existing
ad-hoc signature so macOS permissions have a stable bundle identity, but those
builds are not represented as notarized releases.
